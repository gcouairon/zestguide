...
## Taken from https://github.com/Tramac/awesome-semantic-segmentation-pytorch/blob/master/core/utils/score.py 
import torch
import numpy as np
...
class SegmentationMetric(object):
    ...
    def update(self, preds, labels):
        ...
        def evaluate_worker(self, pred, label):
            ...
            inter, union, cls_visited = batch_intersection_union(pred, label, self.nclass)
...
def batch_intersection_union(output, target, nclass):
    ...
    nbins = nclass
    predict = output
    predict = (predict+1).float()
    target = (target+1).float()
    # print(torch.unique(predict), torch.unique(target))
